    ...
    public class TweakableVector4 : TweakableValueBase<Vector4>
    {
        ...
        protected override void CreateValueControls(TweakerStatus status, int index, float x, float y, float w, float h)
        {
            ...
            new TextControl("W: " + Value.W.ToString("N3", System.Globalization.CultureInfo.InvariantCulture),
                ...
                GetAlpha(status, index), GetTextColor(status, index, 3), status.RootControl);
        }
        ...
        public override void SetFromInputString(TweakerStatus status)
        {
            ...
            Value = new Vector4(status.Index == 0 ? floatValue : Value.X,
                                status.Index == 1 ? floatValue : Value.Y,
                                status.Index == 2 ? floatValue : Value.Z,
                                status.Index == 3 ? floatValue : Value.W);
        }
    }
}
